...
class account_c:
    def __init__(self, exchange = None, name = 'default', apiKey = None, secret = None, password = None, marginMode = None, settleCoin = None, symbol = None, leverage = None, quantity = None)->None:
...
for ac in accounts_data:
    ...
    if( symbol == None ):
        symbol = ""
        print( " * ERROR PARSING ACCOUNT INFORMATION: SYMBOL" )
        continue

    leverage = ac.get('LEVERAGE')
    if( leverage == None ):
        leverage = ""
        print( " * ERROR PARSING ACCOUNT INFORMATION: LEVERAGE" )
        continue

    quantity = ac.get('QUANTITY')
    if( quantity == None ):
        quantity = ""
        print( " * ERROR PARSING ACCOUNT INFORMATION: QUANTITY" )
        continue
    ...
    print( timeNow(), " Initializing account: [", account_id, "] in [", exchange , ']', ' ', symbol)
    try:
        account = account_c( exchange, account_id, api_key, secret_key, password, marginMode, settleCoin, symbol, leverage, quantity)
    except Exception as e:
